...
class Resurtido
{
        ...
        $sql = "
            ...
            LEFT JOIN (
                ...
                WHERE
                    d.almacen_id =
                        :almacen_devolucion_busqueda
                    AND COALESCE(d.tiene_ticket, 0) = 0
                    AND d.estatus IN (
                        'PENDIENTE',
                        'EN_PROCESO'
                    )
    ...
    private function generarFolio(
        ...
        /*
         * crear() bloquea previamente la fila del almacén con
         * FOR UPDATE. Por eso es seguro obtener aquí el siguiente
         * consecutivo aun cuando dos tabletas envíen al mismo tiempo.
         *
         * Se calcula directamente desde los folios existentes para no
         * depender de una tabla auxiliar que puede no estar instalada.
         * RESURTIDO y TICKET siguen teniendo conteos independientes.
         */
        $sqlUltimoFolio = "
            SELECT COALESCE(
                MAX(
            ...
            WHERE
                ...
                AND UPPER(
                    TRIM(
                        COALESCE(tipo_solicitud, 'RESURTIDO')
                    )
                ) = :tipo_solicitud
        ";
